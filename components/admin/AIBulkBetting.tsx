...
const SmartBulkBetting = () => {
    ...
    const activeClients = clients.filter(c => c.role === Role.Client && c.isActive);
    ...
    const handleParseMessage = () => {
        ...
        try {
            const { parsedBets: bets, errors } = parseMessageWithRules(message);
    ...
    const handlePlaceBets = async () => {
        if (!selectedClientId) {
            setNotification({ type: 'error', message: "Please select a client." });
            return;
        }
        if (!selectedDrawId) {
        ...
        try {
            ...
            const result = await placeBetsForClient(betsToPlace, selectedClientId);
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Paste the full message from the client here, then click 'Parse Message' below."
                            className="w-full bg-brand-surface border border-brand-secondary rounded-lg py-3 px-4 text-brand-text focus:outline-none focus:ring-2 focus:ring-brand-primary font-mono"
                            disabled={isLoading}
                        />
                         <button 
                            onClick={handleParseMessage} 
                            disabled={isLoading || !message.trim() || !selectedClientId || !selectedDrawId}
                    ...
                    <div>
                        ...
                        <form onSubmit={handleManualAdd} className="flex flex-wrap gap-4 items-end bg-brand-surface p-4 rounded-lg border border-brand-secondary">
                            <div className="flex-grow">
                                <label className="text-xs font-bold text-brand-text-secondary">Number (1-4 digits)</label>
                                <input type="text" value={manualNumber} onChange={e => setManualNumber(e.target.value.replace(/[^0-9]/g, ''))} maxLength={4} className="w-full bg-brand-bg border border-brand-secondary rounded-lg py-2 px-3 text-brand-text focus:outline-none focus:ring-2 focus:ring-brand-primary font-mono text-lg" />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-brand-text-secondary">Stake</label>
                                <input type="number" value={manualStake} min="1" step="1" onChange={e => setManualStake(e.target.value)} className="w-full bg-brand-bg border border-brand-secondary rounded-lg py-2 px-3 text-brand-text focus:outline-none focus:ring-2 focus:ring-brand-primary" />
                            </div>
                    ...
                    {parsedBets.length > 0 && (
                        <div className="bg-brand-surface p-4 rounded-lg border border-brand-secondary">
                            ...
                            <div className="mt-4 pt-4 border-t border-brand-secondary flex flex-col md:flex-row justify-between items-center gap-4">
                               ...
                               <button onClick={handlePlaceBets} disabled={isLoading || !selectedClientId || !selectedDrawId} className="w-full md:w-auto bg-brand-primary hover:bg-yellow-400 text-brand-bg font-bold py-3 px-6 rounded-lg disabled:bg-brand-secondary disabled:cursor-not-allowed transition-colors">
...
export default SmartBulkBetting;